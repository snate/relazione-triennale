...
Durante l'attività di stage era richiesto lo sviluppo di una \gloss{direttiva}
...
Questi strumenti sono stati inseriti in un nuovo modulo, chiamato \gloss{kpi}
...
etc.). %Aggiungere il documento fatto per descrivere quando vanno utilizzati?
...
\hline
\end{tabular}
\captionof{table}{Requisiti individuati per l'avvio di istanze di processi}
...
\hline
\end{tabular}
\captionof{table}{Requisiti individuati per le \gloss{api} Scrum in JPAUtil}
...
\hline
\end{tabular}
\captionof{table}{Requisiti individuati per il collegamento all'area Forum}
...
\glosspl{direttiva} in modo tale da creare delle componenti riusabili nei moduli
...
\item L'introduzione di una \gloss{direttiva} riusabile nei vari moduli in cui si
...
specifico per questi, \gloss{kpi}.
...
processi, sono state realizzate delle \gloss{api} che permettessero lo stesso
...
con le \gloss{api} ed un account legato a persona reale, è stata preferita la
...
a quello principale nel \gloss{repository} che ho utilizzato per gestire lo
sviluppo del \FREND{}.
...
Per lo sviluppo delle chiamate \gloss{rest} in uscita dal \BKEND{} è stato
